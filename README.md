...
The app is a single static page (`docs/index.html` + `docs/app.js`), hosted on GitHub Pages at https://sebastiansells13-bot.github.io/the-monthly-table/, backed by Firebase (Firestore + Storage), project `the-monthly-table`, via the modular JS SDK loaded from `gstatic.com`.
...
- `docs/index.html` includes `[hidden]{display:none!important}` explicitly — without it, a class-based `display` rule at equal CSS specificity to `[hidden]` will beat the browser's native hidden-attribute handling and the element never actually hides.
